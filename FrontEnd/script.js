...
const elements = {
    ...
    groupMatricula: $('#groupMatricula'),
    groupDisciplina: $('#groupDisciplina'),
    formTipo: $('#formTipo'),
...
async function apiRequest(endpoint, method = 'GET', body = null) {
    ...
    for (const baseUrl of API_BASE_CANDIDATES) {
        try {
            ...
            lastError = error;
            console.warn(`[API] Falha em ${baseUrl}:`, error.message || error);
        }
    }
...
async function createEscola(data) {
...
async function updateEscola(id, data) {
...
function updateStats() {
    ...
    if (elements.statsEscolas) {

        elements.statsEscolas.textContent =
            escolas.length;
...
window.editItem =
    function (id, tipo) {
        ...
        if (elements.groupMatricula) {
            elements.groupMatricula.style.display = tipo === 'aluno' ? 'block' : 'none';
        }

        if (elements.groupDisciplina) {
            elements.groupDisciplina.style.display = tipo === 'professor' ? 'block' : 'none';
        }
        ...
        if (elements.formMatricula) {
            elements.formMatricula.value =
                item.matricula ?? item.alunoMatricula ?? '';
        }

        if (elements.formDisciplina) {
            elements.formDisciplina.value =
                item.disciplina ?? item.professorDisciplina ?? '';
        }
...
window.deleteItem =
    async function (id, tipo) {
        ...
        const confirmed =
            confirm(
                `Deseja realmente excluir esta ${label}?`
            );
        ...
        try {
            ...
            showToast(
                `${capitalize(label)} ${tipo === 'escola' ? 'excluída' : 'excluído'} com sucesso!`,
                'success'
            );
...
async function handleFormSubmit(event) {
    ...
    const matricula =
        elements.formMatricula
            ?.value
            ?.trim() ?? '';

    const disciplina =
        elements.formDisciplina
            ?.value
            ?.trim() ?? '';
    ...
    if (tipo === 'aluno' && !matricula) {
        showToast('Informe a matrícula do aluno.', 'error');
        elements.formMatricula?.focus();
        return;
    }

    if (tipo === 'professor' && !disciplina) {
        showToast('Informe a disciplina do professor.', 'error');
        elements.formDisciplina?.focus();
        return;
    }
